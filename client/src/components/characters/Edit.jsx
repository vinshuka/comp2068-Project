...
const Edit = function (props) {
    ...
    useEffect(() => {
        (async () => {
            const characterResp = await Axios.get(`/api/characters/${id}`);
            if (characterResp.status === 200) setInputs(characterResp.data);
        })();
    }, []);
    ...
    const handleSubmit = async event => {
        ...
        try {
            const resp = await Axios.post('/api/characters/update', inputs);

            if (resp.status === 200) {
                toast("Once again, Go forth brave hero!", {
                    type: toast.TYPE.SUCCESS
                });
                setRedirect(true);
            };
        } catch (error) {
            toast("There was an error updating this character...", {
